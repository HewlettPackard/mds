...
public class FilterValLink<Head, K, V> extends	KeyValLink<Head, K, V, K, V> {
  protected final NullaryFunc<? extends Predicate<? super V>> creator;
  protected final boolean keep;

  public FilterValLink(
                       KeyValChain<Head, ? extends K, ? extends V> pred,
                       NullaryFunc<? extends Predicate<? super V>> creator,
                               boolean keep) {
    super(pred);
  ...
  public Flow pipeInto(final Receiver<? super K, ? super V> sink) {
    return pred.pipeInto(new ChainedReceiver<K, V, K, V>(sink) {
      Predicate<? super V> predicate = creator.call();

      @Override
      public boolean receive(K key, V value) {
        if (predicate.test(value) == keep) {
    ...
    return new Context() {
      ...
      public Pair<K, V> produce() throws SourceExhausted {
        while (true) {
          ...
          if (predicate.test(elt.value) == keep) {
