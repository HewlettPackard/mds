...
public abstract class AbstractLongDoubleMap extends AbstractMap {
  ...
  public boolean equals(Object obj) {
    ...
    if (!(obj instanceof AbstractLongDoubleMap)) return false;
  ...
  @Override
  public int hashCode() {
    final class HashProc implements LongDoubleProcedure {
      int hash = 0;

      @Override
      public boolean apply(long first, double second) {
        long n = Double.doubleToLongBits(second);
        hash ^= (first ^ (first >>> 32) ^ n ^ (n >>> 32));
        return true;
      }
    }

    HashProc proc = new HashProc();
    forEachPair(proc);
    return proc.hash;
  }
...
@param valueList the list to be filled with values, can have any size.
   ...
   * This method can be used to iterate over the values of the receiver.
   *
   * @param valueList the list to be filled, can have any size.
   */
  public void values(final DoubleArrayList valueList) {
    valueList.clear();
    forEachKey(
        new LongProcedure() {
          public boolean apply(long key) {
